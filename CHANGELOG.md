...
## v1.3.1 — La Prima HT false positive fix + slider constant fix (May 7, 2026)

### Fixed

- **La Prima (PID 0xF303) : "High Torque is OFF" warning was a false positive.**
  The La Prima does not have a High Torque mode like the Invicta/Forte. Byte 12
  bit 1 of the firmware status is always set on La Prima — it does NOT indicate
  a torque cap. The plugin now:
  - Suppresses the "HT OFF" warning banner for La Prima
  - Shows "N/A (La Prima does not use High Torque mode)" in the Detection card
  - Hides the "Restore High Torque" button in Recovery actions for La Prima
  - Forces `HealthHighTorqueOn = true` internally so no HT-related logic triggers
  - Reports the raw bit value in Dump Diagnostic for troubleshooting

- **Overall Force slider displayed wrong Nm on Forte / La Prima after profile load.**
  `RefreshSlidersFromCache()` and `SavedNm()` used the hardcoded Invicta constant
  (27 Nm) instead of the detected base's factory peak. On La Prima, a profile with
  `main_gain=100` would display `100/100 × 27 = 27 Nm` instead of `12 Nm`. While
  the round-trip through `SetOverallForce` corrected the value on save, the display
  was confusing and could mask stale profiles from the v1.0.17 bug era.

- **CreateProfile captured stale `main_gain` from cache instead of current slider values.**
  Both the "Create" button handler and the auto-match profile creation called
  `CreateProfile()` without first calling `CommitSlidersToCache()`. The new profile
  would snapshot whatever `main_gain` was in `_paramCache` (often a stale value from
  `LoadSettings` / settings.json), not what the user sees on the slider. This was the
  root cause of Chris's La Prima flashing `main_gain=42%` (≈ 5 Nm instead of 12 Nm).
  Fix: `CommitSlidersToCache()` is now called immediately before every `CreateProfile()`.

### Documented

- `IncrementFfbStrength` (FFB Strength +/- buttons) cannot send `main_gain` to
  firmware without triggering the `PEAK *= main_gain/100` rescale on every call.
  The function updates the cache but the live-mode batch intentionally omits
  `main_gain`. Users must click "Save to Wheelbase" to commit force changes.
