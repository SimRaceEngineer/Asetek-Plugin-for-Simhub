# -*- coding: utf-8 -*-
"""
oos_v6.py -- gel puis verdict hors echantillon des regles v6

  python oos_v6.py --geler      # aujourd hui
  python oos_v6.py --verdict    # vers le 1er septembre
...
DEPENDANCE PARTICULIERE A CE GEL, A NE PAS OUBLIER
    ...
    Et le champ accord_matin (pour Z4) vient toujours de profil_jour.csv :
    RELANCER LES DEUX.
"""
import argparse, datetime as dt, hashlib, io, json, math, os, sys
import regles_gelees_v6 as R
...
FR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "regles_gelees_v6.py")
...
def lire_h1():
    ...
    if not os.path.isfile(CSV):
        ...
        print("lance h1_seance.py AVANT oos_v6.py -- sans lui aucun ticket n a")
...
def lire_matin():
    """am_dir, uniquement pour Z4 qui croise avec le gel V5."""
    if not os.path.isfile(MATIN):
        print("/!\\ %s absent : Z4 tombera en fail-open." % MATIN)
        return {}
    out = {}
    for d in _lire_csv(MATIN):
        j, a = (d.get("jour") or "").strip(), (d.get("asset") or "").strip()
        v = (d.get("am_dir") or "").strip().upper()
        if v.startswith("H") or v == "UP":
            v = "UP"
        elif v.startswith("B") or v == "DOWN":
            v = "DOWN"
        else:
            continue
        if j and a:
...
def tableau(lot, titre):
    ...
    inertes = [c for c in res if c != "Z0" and res[c]["n"] == ref["n"]]
    if inertes:
...
def verif_couverture(lot):
    cv = couverture(lot, ["h1_grande"])
    print("couverture 'grande premiere heure' : %.0f%% des tickets" % cv)
    if cv < COUV_MIN:
        print()
        ...
        print("RELANCE h1_seance.py (et profil_jour.py pour Z4), puis ce script.")
        return False
    return True


def geler(a):
    h1, matin = lire_h1(), lire_matin()
    lot = [s for s in charger(sources(a.fichier), h1, matin)
    ...
    print("regles_gelees_v6.py v%s" % R.VERSION)
...
def verdict(a):
    g = a.gel or (sorted(f for f in os.listdir(".")
                         if f.startswith("gel_v6_") and f.endswith(".json")) or [None])[-1]
    if not g:
        print("Aucun gel_v6_*.json. Lance d abord --geler.")
        return 1
    ...
    if empreinte(FR) != gel["sha256"]:
        print()
        print("*** regles_gelees_v6.py A CHANGE DEPUIS LE GEL ***")
    ...
    print("Z5 est le CONTROLE NEGATIF. Son p sera TOUJOURS identique a celui")
    print("de Z1 : c est son complement, la partition testee est la meme. Ce")
    print("qu il faut regarder est son ECART, qui doit etre le miroir de Z1.")
    print("Si Z1 sort positif et Z5 positif aussi, le harnais mesure autre")
