...
def rapport():
    ...
    L.append("")
    L.append("  AUCUN ORDRE REEL. Les regles sont celles de la production,")
    L.append("  lues dans ignition_trader.py et ignition_trader_trail.py :")
    L.append("  churn_regime._analyze, RSI M3 > 50 / < 50, SL filet fixe,")
    L.append("  lot balance/20000, hold-until-reverse pour le 206, et pour")
    L.append("  le 207 les 70 %% du volume coupes au premier break de la")
    L.append("  bougie M2 precedente en profit, le reste jusqu au reverse.")
    L.append("  Une seule difference assumee : le H1 papier est calcule")
    L.append("  comme ses cinq voisins, alors que la production lit la")
    L.append("  cellule de churn -- sans quoi les six durees ne seraient")
    L.append("  pas comparables entre elles, ce qui est tout l objet.")
    L.append("  Le trail du 207 reste en M2 a toutes les durees, comme en")
    L.append("  production : sur H2 et H4 les 70 %% partiront tres tot, et")
    L.append("  ce sera la regle, pas le marche.")
    ...
    L.append("-" * LARG)
    L.append("  Meme entree, deux sorties -- c est l A/B que la production")
    L.append("  fait deja. Si une duree est bonne des DEUX cotes, c est")
    L.append("  l entree qui vaut ; si elle n est bonne que d un cote,")
    L.append("  c est la sortie. Attention au 207 sur H2 et H4 : son trail")
    L.append("  M2 coupe les 70 %% presque tout de suite, ce qui rabote la")
    L.append("  ride que ces durees sont justement censees capturer.")
    L.append("")
